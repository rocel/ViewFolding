...
public class DaSurfaceView2 extends SurfaceView {
	...
	public DaSurfaceView2(Context context, AttributeSet attrs) {
		super(context, attrs);
...
//		decal = 0;
		Log.d(this.getClass().getName(),DECAL + " | decal = "+ decal);
        ...
        if(position%2==0){
        	matrix.preTranslate(-mWidth+DECAL, -mHeight/2);
        	matrix.postTranslate(startFrom+mWidth-decal+DECAL, mHeight/2);
        } else {
        	matrix.preTranslate(+DECAL, -mHeight/2);
        	matrix.postTranslate(startFrom-decal+DECAL, mHeight/2);
        }
